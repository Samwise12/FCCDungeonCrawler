...
export default () => {
	...
	const createRoomsFromSeed = (grid, {x, y, width, height}, range = c.ROOM_SIZE_RANGE) => {
		...
		const north = { height: random(min, max), width: random(min, max) };
		north.x = random(x, x + width - 1);
		north.y = y - north.height - 1;
		north.doorx = random(north.x, (Math.min(north.x + north.width, x + width)) - 1);
		north.doory = y - 1;
		roomValues.push(north);

		const east = { height: random(min, max), width: random(min, max) };
		east.x = x + width + 1;
		east.y = random(y, height + y - 1);
		east.doorx = east.x - 1;
		east.doory = random(east.y, (Math.min(east.y + east.height, y + height)) - 1);
		roomValues.push(east);

		const south = { height: random(min, max), width: random(min, max) };
		south.x = random(x, width + x - 1);
		south.y = y + height + 1;
		south.doorx = random(south.x, (Math.min(south.x + south.width, x + width)) - 1);
		south.doory = y + height;
		roomValues.push(south);

		const west = { height: random(min, max), width: random(min, max) };
		west.x = x - west.width - 1;
		west.y = random(y, height + y - 1);
		west.doorx = x - 1;
		west.doory = random(west.y, (Math.min(west.y + west.height, y + height)) - 1);
	...
	for (let i = 0; i < c.GRID_HEIGHT; i++) {
		...
		for (let j = 0; j < c.GRID_WIDTH; j++) {
			grid[i].push({type: 0, opacity: random(0.3, 0.8)});
		}
	}
	...
	const firstRoom = {
		x: random(1, c.GRID_WIDTH - max - 15),
		y: random(1, c.GRID_HEIGHT - max - 15),
